...
class SharedPreference(private val context: Context = TheApp.context) {
   ...
   // endregion
    ...
    fun save(KEY_NAME: String, number: Boolean) {
        val editor: SharedPreferences.Editor = sharedPref.edit()
        editor.putBoolean(KEY_NAME, number)
        editor.apply()
    }

    fun save(KEY_NAME: String, number: Set<String>) {
        val editor: SharedPreferences.Editor = sharedPref.edit()
        editor.putStringSet(KEY_NAME, number)
        editor.apply()
    }
    ...
    fun getValueSetString(KEY_NAME: String): Set<String>? {
        return sharedPref.getStringSet(KEY_NAME, mutableSetOf("Brinda"))
    }
